server <- shinyServer(function(input, output) {
  ...
  output$preview <- renderTable({
    ...
    datafile.tibb <- read.csv_override(input$dataFile$datapath)
    groups.tibb <- read.csv_override(input$groupsFile$datapath)
    head(datafile.tibb)
  })
  ...
  observeEvent(input$action, {
    ...
    ## Create the user-input variables ##
    ...
    ## getwd() okay for package development because
    ## it doesn't create changes to global R environment
    path <- getwd()
      if(input$threads == ""){
        WGCNA::enableWGCNAThreads()
      }else{
        threads <- as.numeric(input$threads)
        WGCNA::enableWGCNAThreads(threads)
        }
    ...
    # Load the data files ----
    allDataFile <- read.csv_override(file = input$dataFile$datapath,
                            fileEncoding = "UTF-8-BOM")
    groupsFile <- read.csv_override(file = input$groupsFile$datapath,
                           fileEncoding = "UTF-8-BOM")
    ...
    sft <- WGCNA::pickSoftThreshold(allDataFile_t,
                             powerVector = powers,
                             RsquaredCut = RCutoff,
                             verbose = 5)
    ...
    adjacency <- WGCNA::adjacency(allDataFile_t,
                                  power = softPower,
                                  type="signed")
    ...
    proTree <- fastcluster::hclust(as.dist(dissTOM), method = "average")
    ...
    mergedClust <- tryCatch({
      mergeModuleEigenproteins(allDataFile_t,
                                      moduleColors = dynamicColors,
                                      cutHeight = MCutHeight)},

      error = function(cond){message("MCutHeight too low!")
    ...
    DendrogramEigenproteins <- WGCNA::plotEigengeneNetworks(MEs,
    ...
    tidyDataFinal <- tidyModuleDataForOutput(dataInput = allDataFile_t,
    ...
    # Get KME - correlation between proteins and eigenproteins ----
    kmes <- WGCNA::signedKME(allDataFile_t, MEs)

    # separate results by modules, order by kME, hub proteins on top,
    # in preparation for writing to excel sheet
    dat.res <- data.frame(allDataFile, moduleColors , kmes)

    list.cluster.dat <- lapply(levels(as.factor(moduleColors)),
          ## abstract function to apply:
          ## moduleColors is converted to a factor, then the factor is used to
          ## get each of the proteins belonging to the corresponding module. Then
          ## the KMEs are also added.
    ...
    ## names of the list are now the module
    names(list.cluster.dat) <- 	levels(as.factor(moduleColors))

    #Calculate the module eigenproteins
    allDataModuleEigenProteins <- WGCNA::moduleEigengenes(expr =  allDataFile_t,
                                                          colors = moduleColors)
    #Quality control on the sample clustering
    sampleTree <- fastcluster::hclust(dist(allDataFile_t), method = "average")

    #Get the Eigenproteins for each module ready to print
    EigenProteins <- tibble::tibble(
                                rownames(allDataModuleEigenProteins$eigengenes),
                                allDataModuleEigenProteins$eigengenes)
    EigenProteinsColumnNumber <- length(colnames(EigenProteins))

    EigenProteins_tidy <- magrittr::`%>%`(EigenProteins,
      tidyr::gather(key = "Samples",
                    value = "ModuleEigenproteins",
                    2:EigenProteinsColumnNumber))

    colnames(EigenProteins_tidy) <- c("Experiment",
                                      "Module",
                                      "ModuleEigenprotein")
    EigenProteinsFin <- dplyr::left_join(EigenProteins_tidy,
                                         tibble::tibble(groupsFile),
                                         "Experiment")
    ...
    # Write the files
    # module memberships
    userInputDatabase <- readr::read_tsv(input$databaseFile$datapath)
    ...
    EigenproteinsWide <- tidyr::spread(data = EigenProteinsFin,
                                key = `Module`,
    ...
    # Create the .pdfs
    EigenProteinsFin <- tibble::as_tibble(EigenProteinsFin)
